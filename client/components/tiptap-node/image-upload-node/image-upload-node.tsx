...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    if (file.size > options.maxSize) {
      const error = new Error(
        `La taille du fichier dépasse le maximum autorisé (${options.maxSize / 1024 / 1024}MB)`
      )
    ...
    try {
      ...
      if (!url) throw new Error('Échec du téléchargement : aucune URL renvoyée')
      ...
      if (!abortController.signal.aborted) {
        ...
        options.onError?.(error instanceof Error ? error : new Error('Échec du téléchargement'))
      }
      return null
    }
  }
  ...
  const uploadFiles = async (files: File[]): Promise<string[]> => {
    if (!files || files.length === 0) {
      options.onError?.(new Error('Aucun fichier à télécharger'))
      return []
    }

    if (options.limit && files.length > options.limit) {
      options.onError?.(
        new Error(`Maximum ${options.limit} file${options.limit === 1 ? '' : 's'} permis`)
      )
      return []
    }
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      <span className="tiptap-image-upload-text">
        <em>Cliquez pour ajouter</em> ou glissez‑déposez
      </span>
      <span className="tiptap-image-upload-subtext">
        Maximum {limit} fichier{limit === 1 ? '' : 's'}, {maxSize / 1024 / 1024}Mo chacun.
      </span>
    </div>
  </>
)
